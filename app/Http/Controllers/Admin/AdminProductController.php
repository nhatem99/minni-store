    ...
    function postAdd(Request $request) {
        ...
        try {
            ...
            if ($request->hasFile('feature_image2')) {
                $dataImg = uploadImage($request->feature_image2, 'product/img2');
                $dataInsert['feature_image2'] = $dataImg['file_path'];
                // $dataInsert['image_post_name'] = $dataImg['file_name'];
            }
            ...
            if (!empty($request->name_color)) {
                ...
                //Insert data color
                if (!empty($request->name_size)) {
                    foreach ($request->name_size as $size) {
                        $sizeCreate = Size::firstOrCreate(['name' => $size]);
                        $sizeIds[] = $sizeCreate->id;
                    }
                }
