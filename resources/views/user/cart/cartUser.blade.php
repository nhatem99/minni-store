...
@section('content')
    <div>
        <h1 style="font-size: 19px; color: brown;margin:25px 0px; font-weight:bold">Thông tin đơn hàng</h1>
                ...
                @if ($orderDetail->count() > 0)
                    ...
                    @foreach ($orderDetail as $item)
                        <tr>
                            <td scope="row">{{ $t++ }}</td>
                            <td>{{ $item->order_id }}</td>
                            <td width="10%"><img src="{{ asset($item->productDetails->feature_image) }}" alt="">
                            </td>
